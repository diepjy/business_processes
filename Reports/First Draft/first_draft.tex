...
\begin{abstract}
Trying to verify if a workflow is satisfiable given that there are constraints and restrictions that are placed on how and if a task can be executed is a hard problem. As businesses and companies expand, an interesting problem needs to be solved: can all the relevant tasks be executed without breaking these constraints? Otherwise there might be a financial loss. \\

We will try and solve this problem using rules written in first order predicate logic to generalise the workflow and generate the relevant rules and axioms to test the satisfiability. We will then see if a set of users can be allocated to and execute the relevant tasks. \\

We developed an application which can handle a user input workflow with constraints and check whether the workflow is still satisfiable with the given constraints.
\end{abstract}
